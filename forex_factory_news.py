...
# ── Risque variable ────────────────────────────────────────────
MAX_RISK_DOLLARS = 800.0      # Risque max atteint quand RR = MIN_RR (0.2)
...
MAX_OPEN_TRADES = 3   # Nombre max de trades ouverts en même temps (toutes paires)
# ──────────────────────────────────────────────────────────────

# =========================
# Position auto-close settings
# =========================
POSITION_MAX_AGE_MINUTES     = 60 * 24
ENABLE_AUTO_CLOSE_PROFIT_AGED = True
MIN_PROFIT_TO_CLOSE           = 0.0
...
# Risque variable
# =========================
def compute_risk(rr: float,
                 max_risk: float = MAX_RISK_DOLLARS,
                 min_rr: float   = MIN_RR,
                 strategy: str   = RISK_STRATEGY) -> float:
    """
    Retourne le risque en dollars selon le RR.
      p = min_rr / rr → 1.0 quand rr == min_rr (risque max)
                       → 0.0 quand rr → ∞ (risque nul)
    """
    p = min_rr / rr
    if strategy == 'linear':
        return float(max_risk * p)
    elif strategy == 'exponential':
        return float(max_risk * (p ** 2))
    elif strategy == 'sqrt':
        return float(max_risk * np.sqrt(p))
    else:
...
def get_all_positions():
    """Retourne le nombre total de positions ouvertes sur toutes les paires."""
    total = 0
    for symbol in SYMBOLS:
        try:
            ...
            log.warning(f"[{symbol}] Impossible de récupérer les positions: {e}")
    return total
    ...
    try:
        ...
        for pos in positions:
            ...
            if age_minutes >= POSITION_MAX_AGE_MINUTES and profit > MIN_PROFIT_TO_CLOSE:
                log.info(f"[{symbol}] Position {ticket} age={age_minutes:.1f}min, "
                         f"profit=${profit:.2f} -> closing")
                close_position(symbol, ticket)
            else:
                log.debug(f"[{symbol}] Position {ticket} age={age_minutes:.1f}min, "
                          f"profit=${profit:.2f} -> not closing")
    except Exception as e:
...
# =========================
def process_symbol(symbol, n_open_trades: int) -> int:
    """
    Traite un symbole.
    Retourne 1 si un trade a été ouvert, 0 sinon.
    n_open_trades : nombre de trades déjà ouverts au moment de l'appel.
    """
    try:
        # ── Filtre MAX_OPEN_TRADES cross-paires ────────────────
        if n_open_trades >= MAX_OPEN_TRADES:
            log.info(f"[{symbol}] MAX_OPEN_TRADES ({MAX_OPEN_TRADES}) atteint - skip")
            return 0
        # ──────────────────────────────────────────────────────
        ...
        # ── Zone RR pénalisée ──────────────────────────────────
        pen_lo, pen_hi    = PENALIZED_RR_ZONE
        in_penalized_zone = pen_lo <= rr <= pen_hi
        if in_penalized_zone and PENALIZED_RISK_FACTOR == 0.0:
            log.info(f"[{symbol}] RR={rr:.2f} dans zone penalisee [{pen_lo}-{pen_hi}] - skip")
            return 0
        # ──────────────────────────────────────────────────────

        # ── Risque dynamique ───────────────────────────────────
        risk_dollars = compute_risk(rr)
        if in_penalized_zone:
            risk_dollars *= PENALIZED_RISK_FACTOR
        # ──────────────────────────────────────────────────────
        ...
        log.info(
            ...
            f"Risque={risk_dollars:.0f}$ (strategie={RISK_STRATEGY}) | "
            f"Open trades={n_open_trades}/{MAX_OPEN_TRADES}"
        )
...
def run_all_symbols():
    ...
    log.info(
        f"Strategie risque: {RISK_STRATEGY} | Risque max: {MAX_RISK_DOLLARS}$ @ RR={MIN_RR} | "
        f"Threshold: {CONFIDENCE_THRESHOLD} | MIN_RR: {MIN_RR} | "
        f"Zone penalisee: RR[{PENALIZED_RR_ZONE[0]}-{PENALIZED_RR_ZONE[1]}] x{PENALIZED_RISK_FACTOR}"
    )
    log.info("=" * 50)

    # Compter les positions ouvertes cross-paires AVANT de traiter les signaux
    n_open = get_all_positions()
    log.info(f"Positions ouvertes (cross-paires): {n_open}/{MAX_OPEN_TRADES}")

    for symbol in SYMBOLS:
        opened = process_symbol(symbol, n_open_trades=n_open)
        n_open += opened   # mise à jour optimiste en cours de tick
        time.sleep(1)
